...
export class LiveController {
  ...
  async createStream(
    ...
    if (!streamerId || !title || !identityType || !identityId) {
      throw new BadRequestException(
        "Vui lòng điền đầy đủ thông tin tạo stream",
      );
    }
    return this.liveService.createStream(
      streamerId,
      title,
      identityType,
      identityId,
  ...
  async endStream(
    @Param("id") id: string,
    @Query("violation") violation?: string,
  ) {
    const isViolation = violation === "true";
    return this.liveService.endStream(id, isViolation);
  }
  ...
  async postMessage(
    ...
    if (!senderId || !content) {
      throw new BadRequestException(
        "Thông tin người gửi hoặc nội dung không hợp lệ",
      );
    }
  ...
  async donate(
    ...
    if (!viewerId || !amount || amount <= 0) {
      throw new BadRequestException(
        "Thông tin người quyên góp hoặc số tiền không hợp lệ",
      );
    }
