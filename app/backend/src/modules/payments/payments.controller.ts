...
export class PaymentsController {
  ...
  async handleSePayWebhook(
    ...
    try {
      ...
      throw new UnauthorizedException(
        err.message || "Xác thực Webhook SePay thất bại",
      );
    }
  }
  ...
  async verifySuccess(
    ...
    if (order.videoId.toString() !== videoId) {
      throw new BadRequestException(
        "Đơn hàng này không khớp với video bạn đang truy cập",
      );
    }
    ...
    if (order.status === "SUCCESS") {
      ...
      await this.paymentsService.addPurchasedVideo(
        userId,
        videoId,
        false,
        order.amount,
      ...
      return {
        status: "success",
        message: "Video already unlocked by Webhook",
      };
    }
    ...
    try {
      ...
      throw new BadRequestException(
        err.message || "Xác thực thanh toán thất bại",
      );
    }
  }
