...
export class SupportService {
  ...
  async onModuleInit() {
    ...
    if (count > 0) {
      console.log(
        `[Support] Đã khôi phục và migrate thành công ${count} ticket cũ.`,
      );
    } else {
  ...
  async getUserTickets(userId: string) {
    const tickets = await this.ticketModel
      ...
      .sort({ updatedAt: -1 });
    console.log(
      `[Support] Lấy ${tickets.length} tickets cho user ${userId}. Ticket đầu tiên có ${tickets[0]?.messages?.length || 0} tin nhắn.`,
    );
    return tickets;
  }
