...
export class UsersController {
  ...
  @Post("register")
  async register(
    @Body() registerDto: { username: string; phone: string; password: string },
  ) {
    try {
  ...
  async updatePreferences(
    @Body("userId") userId: string,
    @Body("premium_show_avatar_aura") premium_show_avatar_aura?: boolean,
    @Body("premium_show_comment_aura") premium_show_comment_aura?: boolean,
  ) {
    try {
      const res = await this.usersService.updatePreferences(userId, {
        premium_show_avatar_aura,
        premium_show_comment_aura,
      });
      return res;
    } catch (error: any) {
      return { success: false, message: error.message };
  ...
  async donate(
    ...
    try {
      const res = await this.usersService.donate(senderId, receiverId, amount);
      return res;
    } catch (error: any) {
      return { success: false, message: error.message };
    }
