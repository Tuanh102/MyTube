...
export default function AdminSearchPage() {
    ...
    const fetchResults = useCallback(async (q: string) => {
        ...
        setError(null);
        try {
            const res = await fetch(`/api/admin/smart-search?q=${encodeURIComponent(q)}&role=admin`);
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        if (inputValue.trim()) {
            router.push(`/admin/search?q=${encodeURIComponent(inputValue.trim())}`);
        }
    };
    ...
    return (
        <div className="min-h-screen bg-rose-50/20 dark:bg-[#020202] text-zinc-800 dark:text-zinc-100 font-sans">
            {/* Header */}
            <header className="sticky top-0 z-50 h-16 border-b border-rose-100 dark:border-red-950/20 flex items-center gap-4 px-8 bg-white/90 dark:bg-[#090909]/90 backdrop-blur-md">
                <button
                    onClick={() => router.back()}
                ...
                <span className="text-xs text-zinc-400 dark:text-white/25 font-medium hidden md:block">Admin Panel</span>
            </header>
