  ...
  callbacks: {
    ...
    async jwt({ token, user, account, trigger, session }) {
      if (user) {
        ...
        token.premium_show_avatar_aura = (user as any).premium_show_avatar_aura !== false;
        token.premium_show_comment_aura = (user as any).premium_show_comment_aura !== false;
        if (account) {
        ...
        if (account && account.provider !== "credentials") {
          try {
            const res = await fetch(`http://127.0.0.1:5000/users/profile/${user.id}`, { cache: 'no-store' });
            if (res.ok) {
              ...
              if (dbUser && dbUser._id) {
                ...
                token.premium_show_avatar_aura = dbUser.premium_show_avatar_aura !== false;
                token.premium_show_comment_aura = dbUser.premium_show_comment_aura !== false;
    ...
    async session({ session, token }) {
      if (session.user) {
        ...
        if (!userId) {
          ...
          if (avatar.includes("facebook") || avatar.includes("fbsbx.com")) {
            ...
            if (match && match[1]) {
              const fbId = match[1];
              try {
                const fbRes = await fetch(`http://127.0.0.1:5000/users/profile/${fbId}`, { cache: 'no-store' });
                if (fbRes.ok) {
        ...
        if (!userId && session.user.email) {
          try {
            const emailRes = await fetch(`http://127.0.0.1:5000/users/profile-by-email?email=${encodeURIComponent(session.user.email)}`, { cache: 'no-store' });
            if (emailRes.ok) {
        ...
        if (!userId && session.user.name) {
          try {
            const userRes = await fetch(`http://127.0.0.1:5000/users/profile-by-username?username=${encodeURIComponent(session.user.name)}`, { cache: 'no-store' });
            if (userRes.ok) {
        ...
        if (userId) {
          try {
            let res = await fetch(`http://127.0.0.1:5000/users/profile/${userId}`, { cache: 'no-store' });
            let dbUser = null;
            if (res.ok) {
              const text = await res.text();
            ...
            if (dbUser) {
              ...
              (session.user as any).premium_show_avatar_aura = dbUser.premium_show_avatar_aura !== false;
              (session.user as any).premium_show_comment_aura = dbUser.premium_show_comment_aura !== false;
            } else {
              ...
              (session.user as any).premium_show_avatar_aura = token.premium_show_avatar_aura !== false;
              (session.user as any).premium_show_comment_aura = token.premium_show_comment_aura !== false;
            }
          } catch (err: any) {
            (session.user as any).balance = token.balance || 0;
            (session.user as any).is_premium = token.is_premium || false;
