...
export default function PremiumPage() {
  ...
  useEffect(() => {
    async function fetchPackages() {
      try {
        const res = await fetch('/api/payments/premium-packages');
        if (res.ok) {
  ...
  useEffect(() => {
    if (user) {
      setShowVipGlow(user.premium_show_comment_aura !== false);
      setEnableCrownRing(user.premium_show_avatar_aura !== false);
    }
  }, [user?.premium_show_comment_aura, user?.premium_show_avatar_aura]);

  const handleToggleVipGlow = async () => {
    const newValue = !showVipGlow;
    setShowVipGlow(newValue);
    if (user?.id) {
      await updatePreferencesAction(user.id, { premium_show_comment_aura: newValue });
      if (update) {
        await update({
          ...session,
          user: {
            ...session.user,
            premium_show_comment_aura: newValue
          }
        });
      }
    }
  ...
  const handleToggleCrownRing = async () => {
    const newValue = !enableCrownRing;
    setEnableCrownRing(newValue);
    if (user?.id) {
      await updatePreferencesAction(user.id, { premium_show_avatar_aura: newValue });
      if (update) {
        await update({
          ...session,
          user: {
            ...session.user,
            premium_show_avatar_aura: newValue
          }
        });
      }
    }
  ...
  const handleSelectPackage = async (packageType: string) => {
    ...
    let targetKey = packageType;
    if (packageType === 'month') targetKey = 'PREMIUM_MONTH';
    else if (packageType === '6months') targetKey = 'PREMIUM_6MONTHS';
    else if (packageType === 'year') targetKey = 'PREMIUM_YEAR';

    const pkg = dbPackages.find(p => p.key === targetKey) || {
      key: targetKey,
      price: targetKey === 'PREMIUM_MONTH' ? 25000 : targetKey === 'PREMIUM_6MONTHS' ? 135000 : 250000,
      name: targetKey === 'PREMIUM_MONTH' ? 'Gói Premium 1 Tháng' : targetKey === 'PREMIUM_6MONTHS' ? 'Gói Premium 6 Tháng' : 'Gói Premium 12 Tháng'
    };

    const amount = pkg.price;
    const videoId = pkg.key;
      ...
      <div className="max-w-6xl w-full flex flex-col lg:flex-row gap-10 items-start mb-12">
        ...
        <div className="w-full lg:w-[65%] grid grid-cols-1 md:grid-cols-3 gap-6">
          {(dbPackages.length > 0 ? dbPackages : [
            { key: 'PREMIUM_MONTH', name: 'Gói 1 Tháng', price: 25000, durationDays: 30, description: 'Trải nghiệm ngắn hạn' },
            { key: 'PREMIUM_6MONTHS', name: 'Gói 6 Tháng', price: 135000, durationDays: 180, description: 'Phổ biến hàng đầu' },
            { key: 'PREMIUM_YEAR', name: 'Gói 1 Năm', price: 250000, durationDays: 360, description: 'Lựa chọn tối ưu nhất' }
          ]).map((pkg) => {
            const isMonth = pkg.key === 'PREMIUM_MONTH' || pkg.durationDays === 30;
            const is6Months = pkg.key === 'PREMIUM_6MONTHS' || pkg.durationDays === 180;

            if (isMonth) {
              return (
                <div key={pkg.key} className="bg-[#181818]/90 border border-white/10 rounded-3xl p-6 flex flex-col justify-between shadow-2xl relative overflow-hidden transition-all duration-300 hover:border-white/20 group hover:scale-[1.01] min-h-[380px]">
                  <div>
                    <div className="flex justify-between items-start mb-6">
                      <div>
                        <h3 className="text-base font-black text-white uppercase tracking-wider">{pkg.name}</h3>
                        <p className="text-zinc-400 text-[10px] mt-1">{pkg.description || 'Trải nghiệm ngắn hạn'}</p>
                      </div>
                    ...
                    <div className="mb-6 flex items-baseline gap-1">
                      <span className="text-2xl font-black text-white">{pkg.price.toLocaleString('vi-VN')}đ</span>
                      <span className="text-zinc-500 text-[10px] font-semibold">/ {pkg.durationDays} ngày</span>
                    </div>
                    <ul className="space-y-3 mb-6">
                      <li className="flex items-center gap-2 text-[11px] text-zinc-300">
                        <Check size={12} className="text-[#00b082]" />
                        <span>Chặn QC trong {pkg.durationDays} ngày</span>
                      </li>
                      <li className="flex items-center gap-2 text-[11px] text-zinc-300">
                        <Check size={12} className="text-[#00b082]" />
            ...
            if (is6Months) {
              return (
                <div key={pkg.key} className="bg-gradient-to-b from-blue-500/10 to-transparent bg-[#181818]/90 border border-blue-500/30 rounded-3xl p-6 flex flex-col justify-between shadow-2xl relative overflow-hidden transition-all duration-300 hover:border-blue-500/50 group hover:scale-[1.01] min-h-[380px]">
                    ...
                    <div className="flex justify-between items-start mb-6">
                      <div>
                        <h3 className="text-base font-black text-blue-400 uppercase tracking-wider flex items-center gap-1">
                          {pkg.name} <Flame size={14} className="fill-blue-400 text-blue-400" />
                        </h3>
                        <p className="text-zinc-400 text-[10px] mt-1">{pkg.description || 'Phổ biến hàng đầu'}</p>
                      </div>
                    </div>
                    <div className="mb-6">
                      ...
                      <p className="text-[9px] text-blue-400 font-semibold mt-1">Chỉ khoảng {Math.round(pkg.price / (pkg.durationDays / 30)).toLocaleString('vi-VN')}đ / tháng</p>
                    </div>
                    <ul className="space-y-3 mb-6">
                      <li className="flex items-center gap-2 text-[11px] text-zinc-300">
                        <Check size={12} className="text-blue-400" />
                        <span className="font-semibold text-white">Chặn QC trong {pkg.durationDays} ngày</span>
                      </li>
                      <li className="flex items-center gap-2 text-[11px] text-zinc-300">
                        <Check size={12} className="text-blue-400" />
                        <span>Băng thông VIP cực nhanh</span>
                  ...
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      <h3 className="text-base font-black text-amber-400 uppercase tracking-wider flex items-center gap-1">
                        {pkg.name} <Star size={12} className="fill-amber-400" />
                      </h3>
                      <p className="text-zinc-400 text-[10px] mt-1">{pkg.description || 'Lựa chọn tối ưu nhất'}</p>
                    </div>
                  </div>
                  <div className="mb-6">
                    <div className="flex items-baseline gap-1">
                    ...
                    <p className="text-[9px] text-[#00b082] font-semibold mt-1">Chỉ khoảng {Math.round(pkg.price / (pkg.durationDays / 30)).toLocaleString('vi-VN')}đ / tháng</p>
                  </div>
                  <ul className="space-y-3 mb-6">
                    <li className="flex items-center gap-2 text-[11px] text-zinc-300">
