...
export async function updatePreferencesAction(
    userId: string,
    prefs: { premium_show_avatar_aura?: boolean; premium_show_comment_aura?: boolean }
) {
    try {
        const res = await fetch(`${API_URL}/users/update-preferences`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
export async function donateAction(senderId: string, receiverId: string, amount: number) {
    try {
        const res = await fetch(`${API_URL}/users/donate`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        console.error("donateAction error:", err);
        return { success: false, message: String(err) };
    }
}
