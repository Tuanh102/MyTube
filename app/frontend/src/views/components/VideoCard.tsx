      ...
      <div className="flex gap-3 relative">
        <div className="flex-shrink-0 relative">
          ...
          {video.channel_is_verified && (
            <div 
              id={`verified-badge-${video.video_id}`}
        ...
        <div className="flex-1 pr-4 min-w-0">
          ...
          {video.isLive ? (
            <p className="text-xs text-white/60 truncate flex items-center gap-1">
              <span>{video.channel_name || 'Người phát sóng'}</span>
              {video.channel_is_verified && (
            ...
            <Link href={`/channel/${video.channel_id || '#'}`}>
              <p className="text-xs text-white/60 hover:text-white transition-colors truncate flex items-center gap-1">
                <span>{video.channel_name || 'Kênh hệ thống'}</span>
                {video.channel_is_verified && (
                  <span className="w-2.5 h-2.5 rounded-full bg-blue-500 flex items-center justify-center text-[7px] font-black text-white" title="Kênh đã xác minh">✓</span>
                )}
              </p>
