...
import { Plus, Video, Tv, Layout, Settings, Trash2, Edit2, PlayCircle, BarChart2, Users, MessageSquare, Eye, Search, ThumbsUp, ThumbsDown, Calendar, HelpCircle, Send, Zap, ShieldCheck, ArrowUpCircle, ArrowDownCircle, DollarSign, CreditCard, AlertTriangle } from 'lucide-react';
...
export default function StudioPage() {
  ...
  const handleTicketClick = async (ticket: any) => {
    ...
    if (!ticket.isReadByUser) {
      try {
        await fetch(`/api/support/mark-read/${ticket._id}`, { method: 'POST' });
        ...
        {activeTab === 'overview' && overviewData ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="bg-[#1a1a1a] border border-white/10 p-6 rounded-2xl hover:border-green-500/30 transition group cursor-pointer" onClick={() => setActiveTab('revenue')}>
                ...
                <div className="flex flex-col gap-1">
                  <p className="text-3xl font-bold text-green-500">{overviewData.summary.balance?.toLocaleString('vi-VN')} <span className="text-[10px] text-white/40">Khả dụng</span></p>
                  {overviewData.summary.frozenBalance > 0 && (
                    <p className="text-sm font-semibold text-yellow-500">+{overviewData.summary.frozenBalance?.toLocaleString('vi-VN')} <span className="text-[10px] text-white/40">Đóng băng</span></p>
                  )}
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {channels.map(channel => (
              <div key={channel._id} className={`bg-[#1a1a1a] border rounded-2xl p-6 transition group ${channel.status === 'BANNED' ? 'border-red-500/60 shadow-lg shadow-red-500/10' : 'border-white/10 hover:border-red-500/50'}`}>
                  ...
                  <div>
                    <h3 className="text-white font-bold text-lg flex items-center gap-2">
                      ...
                      {channel.status === 'BANNED' && (
                        <span className="bg-red-600 text-white text-[9px] font-bold px-1.5 py-0.5 rounded uppercase">Banned</span>
                      )}
                    </h3>
                ...
                <p className="text-white/60 text-sm line-clamp-2 mb-4 h-10">{channel.description || 'Chưa có mô tả'}</p>
                
                {/* Cảnh báo gậy và cấm livestream */}
                <div className="mb-6 space-y-2">
                  {channel.status === 'BANNED' ? (
                    <div className="bg-red-500/15 text-red-400 border border-red-500/30 text-xs font-bold px-3 py-2 rounded-xl text-center uppercase tracking-wider">
                      Kênh đã bị khóa vĩnh viễn (3 gậy)
                    </div>
                  ) : (
                    <>
                      <div className="flex justify-between items-center text-xs bg-white/5 px-3 py-2 rounded-xl">
                        <span className="text-white/40">Cảnh cáo vi phạm:</span>
                        <span className={`font-black uppercase tracking-wider text-[10px] px-2 py-0.5 rounded ${channel.strikes > 0 ? 'bg-amber-500/10 text-amber-500 border border-amber-500/20' : 'bg-green-500/10 text-green-500 border border-green-500/20'}`}>
                          {channel.strikes || 0}/3 gậy
                        </span>
                      </div>
                      
                      {channel.banExpiresAt && new Date(channel.banExpiresAt) > new Date() && (
                        <div className="bg-amber-500/10 text-amber-400 border border-amber-500/20 text-[11px] px-3 py-2 rounded-xl text-center leading-relaxed">
                          Tạm cấm phát trực tiếp đến: <strong className="block text-white font-bold">{new Date(channel.banExpiresAt).toLocaleString('vi-VN')}</strong>
                        </div>
                      )}
                    </>
                  )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 bg-gradient-to-br from-green-600/20 to-zinc-900 border border-green-500/20 rounded-3xl p-8 relative overflow-hidden">
                <div className="relative z-10">
                  ...
                  <h2 className="text-5xl font-black text-white mb-4 tracking-tighter">{overviewData?.summary.balance?.toLocaleString('vi-VN')} <span className="text-2xl text-white/40">VNĐ</span></h2>
                  {overviewData?.summary?.frozenBalance > 0 && (
                    <div className="mb-6 bg-white/5 border border-white/5 rounded-2xl p-4 flex items-center justify-between max-w-md">
                      <div className="flex items-center gap-2 text-yellow-500 text-sm font-bold">
                        <AlertTriangle size={16} />
                        <span>Số tiền đang đóng băng (chờ duyệt):</span>
                      </div>
                      <span className="text-white font-extrabold text-sm">{overviewData?.summary?.frozenBalance?.toLocaleString('vi-VN')} VNĐ</span>
                    </div>
                  )}
