...
import GithubProvider from "next-auth/providers/github"; // Nhập GithubProvider
import DiscordProvider from "next-auth/providers/discord"; // Nhập DiscordProvider
...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    ...
    async session({ session, token }) {
      // Ghi nhật ký chi tiết NextAuth hoạt động (Đã tắt ghi file vật lý để bảo vệ dung lượng ổ đĩa)
      ...
      if (session.user) {
        ...
        if (userId) {
          try {
            ...
            // Profile fetch logged in terminal console only
            ...
            // Session callback exception logged to console
