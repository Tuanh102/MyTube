...
export default function LiveViewerPage() {
  ...
  const handleDonateSubmit = async (e: React.FormEvent) => {
    ...
    if (user.balance < finalAmount) {
      const confirmDeposit = window.confirm(`Số dư ví của bạn (${user.balance?.toLocaleString('vi-VN')} VNĐ) không đủ để donate ${finalAmount.toLocaleString('vi-VN')} VNĐ.\n\nBạn có muốn đi đến trang Nạp tiền ví số dư ảo ngay lập tức không?`);
      if (confirmDeposit) {
        ...
        <div className="lg:col-span-8 flex flex-col gap-4">
          ...
          <div className={`border rounded-3xl p-6 backdrop-blur-md flex items-center justify-between transition-all duration-300 ${
            ...
            <div className="space-y-1">
              ...
              <h3 className={`text-2xl font-black ${isLightMode ? 'text-zinc-950' : 'text-white'}`}>{user?.balance?.toLocaleString('vi-VN') || 0} <span className={`text-sm font-normal ${isLightMode ? 'text-zinc-500' : 'text-white/40'}`}>VNĐ</span></h3>
            </div>
            <button 
          ...
          <div className="bg-[#1a1a1a] border border-white/10 rounded-3xl max-w-lg w-full p-8 relative shadow-2xl animate-in zoom-in-95 duration-300 text-left">
            ...
            <form onSubmit={handleDonateSubmit} className="space-y-6">
              ...
              <div className="bg-white/5 border border-white/5 rounded-2xl p-4 flex justify-between items-center text-xs">
                ...
                <span className="text-white font-extrabold">{user?.balance?.toLocaleString('vi-VN') || 0} VNĐ</span>
              </div>
