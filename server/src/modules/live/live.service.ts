...
export class LiveService {
  constructor(
    @InjectModel(LiveStream.name) private readonly liveStreamModel: Model<LiveStreamDocument>,
    @InjectModel(LiveMessage.name) private readonly liveMessageModel: Model<LiveMessageDocument>,
    @InjectModel(User.name) private readonly userModel: Model<UserDocument>,
    @InjectModel(Channel.name) private readonly channelModel: Model<ChannelDocument>,
  ) {}

  async createStream(streamerId: string, title: string, identityType: 'user' | 'channel', identityId: string) {
    ...
    await this.liveStreamModel.updateMany({ streamerId, isActive: true }, { $set: { isActive: false } });
  ...
  private signals = new Map<string, Map<string, { offer?: string; answer?: string }>>();

  async postSignal(streamId: string, viewerId: string, type: 'offer' | 'answer', sdp: string) {
  ...
  async registerHeartbeat(streamId: string, viewerId: string) {
    ...
    await this.liveStreamModel.findByIdAndUpdate(streamId, { $set: { viewerCount: realCount } });
  }
  ...
  async endStream(streamId: string) {
    ...
    const stream = await this.liveStreamModel.findByIdAndDelete(streamId).exec();
  ...
  async getActiveStreams() {
    ...
    for (const stream of streams) {
      ...
      const streamCreatedAt = (stream as any).createdAt || (stream as any).created_at || new Date();
  ...
  async postMessage(streamId: string, senderId: string, content: string, type: 'chat' | 'donation' = 'chat', donationAmount: number = 0) {
  ...
  async getMessages(streamId: string) {
    return this.liveMessageModel.find({ streamId }).sort({ createdAt: 1 }).limit(100).exec();
  }

  async donate(streamId: string, viewerId: string, amount: number) {
    const stream = await this.liveStreamModel.findById(streamId);
    if (!stream) throw new NotFoundException('Phiên Live không tồn tại');
    if (!stream.isActive) throw new BadRequestException('Phiên Live đã kết thúc');
    ...
    if (viewer.balance < amount) {
      throw new BadRequestException('Số dư ví không đủ để thực hiện quyên góp! Vui lòng nạp thêm tiền.');
    }
    ...
    await this.liveStreamModel.findByIdAndUpdate(streamId, { $inc: { earnings: amount } });
    ...
    const donateMessage = await this.postMessage(streamId, viewerId, alertContent, 'donation', amount);
  ...
  async pinMessage(streamId: string, messageId: string) {
    ...
    const alreadyPinned = stream.pinnedMessages?.some((msg: any) => msg._id?.toString() === messageId);
    if (alreadyPinned) return stream;

    // Giới hạn tối đa 3 bình luận ghim
    if (stream.pinnedMessages && stream.pinnedMessages.length >= 3) {
      throw new BadRequestException('Chỉ được ghim tối đa 3 bình luận. Vui lòng bỏ ghim bình luận cũ trước khi ghim mới!');
    }
  ...
  async unpinMessage(streamId: string, messageId: string) {
    ...
    if (updatedStream && updatedStream.pinnedMessages) {
      const filtered = updatedStream.pinnedMessages.filter((msg: any) => msg._id?.toString() !== messageId);
  ...
  async reportStream(streamId: string, reporterId: string, reason: string, content: string = '') {
    const reporter = await this.userModel.findById(reporterId);
    const reporterName = reporter ? (reporter.username || (reporter as any).name || 'Khán giả') : 'Khán giả';
