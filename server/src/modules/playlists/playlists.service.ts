...
export class PlaylistsService {
  ...
  async getUserPlaylists(userId: string, videoId?: string) {
    const playlists = await this.playlistModel.find({ user: new Types.ObjectId(userId) }).exec();
    
    // Manual resolution to avoid population issues with string IDs
    const result = [];
    for (const pl of playlists) {
      let cover_thumbnail = null;
      let videoCount = pl.videos?.length || 0;
      ...
      result.push({
        ...
        cover_thumbnail,
        has_video: videoId ? (pl.videos.some(v => v.toString() === videoId) ? 1 : 0) : 0
      });
    }
    return result;
  }
  ...
  async getPlaylistVideos(playlistId: string) {
    ...
    const videoIds = playlist.videos.map(v => {
      try { return new Types.ObjectId(v.toString()); } 
      catch (e) { return v; }
    });

    return this.videoModel.find({ _id: { $in: videoIds } })
      .populate('channel')
      .exec();
  }
}
