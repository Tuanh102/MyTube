...
export class ReportsService {
  ...
  async createChannelReport(channelId: string, reporterId: string, reason: string) {
  ...
  async resolveReport(reportId: string, action: 'DELETE_VIDEO' | 'KEEP_VIDEO' | 'DELETE_CHANNEL' | 'KEEP_CHANNEL') {
    ...
    if (action === 'DELETE_VIDEO') {
      ...
      if (channel) {
        ...
        const videos = await this.videoModel.find({ channel: channel._id }).exec();
